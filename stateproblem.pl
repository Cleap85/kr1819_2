...
% State problem
%
% From 5 and 3 gallons jugs, ow we can measure 4 gallons?
%
...
% 3. Empty 5
  ...
  mov(emty5, state(_, G3), state(0, G3)).
  
% 4. Empty 3

  mov(emty3, state(G5, _), state(G5, 0)).
...
% it is true if Path unify with the list of movements needed to go from Initial State to Final State without repeating states in Visited states.
